...
export class Packet {
    ...
    public static fromBytes(bytes: Uint8Array): Packet {
        ...
        // Device ID
        let deviceIdSize = bytes[i++];
        let deviceId = TEXT_DECODER.decode(bytes.subarray(i, i + deviceIdSize));
        i += deviceIdSize;

        // Password
        let passwordSize = bytes[i++];
        let password = TEXT_DECODER.decode(bytes.subarray(i, i + passwordSize));
        i += passwordSize;
        ...
        // Data
        let dataBlocks = new Array((bytes.length - i - 2) / 2);
        for (let j = 0; j < dataBlocks.length; j++) {
            dataBlocks[j] = new DataBlock(bytes[i++], bytes[i++]);
        }

        // Checksum
        let calculated = Packet.checksum(bytes.subarray(2, i));
        let actual = [bytes[i++], bytes[i++]];
